...
export async function onRequestGet({ env }) {
  ...
  const entries = ["/", "/events/", "/private-groups-retreats/", "/what-is-feminine-movement-meditation/", "/privacy/", "/terms/", "/cancellations-refunds/"].map((path) => `<url><loc>${SITE_URL}${path}</loc></url>`);
